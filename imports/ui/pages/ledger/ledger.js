...
Template.ledger.helpers({
  transactions() {
    const month = parseMonthURI(FlowRouter.getParam('month'));
    return Transactions.find({ month }).fetch();
  },
  rowClass() {
    return (this.cleared)
      ? ''
...
Template.ledger.events({
  ...
  'change input.trans-form, click button.trans-form'(event) {
    ...
    const value = (event.type === 'click')
      ? !formEl.hasClass("active")
      : formEl.val();
    const transUpdate = {
  ...
  'click .copy-recurring'() {
    if (confirm('Copy recurring from previous month?')) {
      let month = FlowRouter.getParam("month") || moment().format('MMM_YYYY');
      month = moment(month, 'MMM_YYYY').subtract(1, 'month').format('MM/YY');

      Meteor.call('copyRecurring', Meteor.userId(), month);
    }
  },
});
